...
import { executeQuery, executeProcedure, executeTransaction, generateId } from './config';
...
export async function createUser(userData: CreateUserData): Promise<User> {
  ...
  await executeProcedure('sp_create_user(:p_email, :p_password_hash, :p_display_name, :p_role, :p_user_id)', binds);
...
export async function updateUserProfile(
  ...
  await executeProcedure('sp_update_user_profile(:p_user_id, :p_display_name, :p_phone)', binds);
...
export async function updateUserPassword(userId: string, newPassword: string): Promise<void> {
  ...
  await executeProcedure('sp_change_password(:p_user_id, :p_new_password_hash)', binds);
}
 ...
 */
export async function connectClientToTherapist(clientId: string, therapistId: string): Promise<void> {
  const binds = {
    p_client_id: clientId,
    p_therapist_id: therapistId,
  };
  
  await executeProcedure('sp_connect_client_therapist(:p_client_id, :p_therapist_id)', binds);
}

/**
 * Get therapist's clients
 */
export async function getTherapistClients(therapistId: string): Promise<User[]> {
  const result = await executeQuery<User>(
