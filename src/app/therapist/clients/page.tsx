...
export default function ClientsPage() {
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: 'Hata',
        description: 'Danışanlar getirilemedi.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  async function handleStatusChange(
    ...
    if (!user) {
      toast({
        title: 'Demo Modu',
        description: 'Bu özellik için giriş yapmalısınız.',
        variant: 'destructive',
      });
      return;
    }
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">
            Danışanlarınızın listesini ve ilerlemelerini buradan takip edin.
          </p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Yeni danışanınızın bilgilerini girin. Danışan, bu e-posta adresi
                ile kayıt olduğunda hesabınıza otomatik olarak bağlanacaktır.
              </DialogDescription>
            </DialogHeader>
            <Form {...form}>
              <form
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          disabled={client.status === 'Davet Edildi'}
                        >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() =>
                                handleStatusChange(client.id, 'Aktif')
                              }
                            >
                              Aktif Yap
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() =>
                                handleStatusChange(client.id, 'Pasif')
                              }
                            >
                              Pasif Yap
                            </DropdownMenuItem>
