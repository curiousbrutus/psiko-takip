...
export default function ClientProfilePage() {
  ...
  const handleAssignTask = async () => {
    if (isDemoMode) {
      toast({
        title: 'Demo Modu',
        description:
          'Bu özellik için gerçek bir kullanıcı ile giriş yapmalısınız.',
      });
      return;
    }
  ...
  const handleAssignAssessment = async (
    testName: 'GAD-7' | 'PHQ-9' | 'TherapeuticAlliance'
  ) => {
    if (isDemoMode) {
      toast({
        title: 'Demo Modu',
          ...
          <CardHeader className="flex flex-row items-center gap-6 space-y-0">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                Genel ilerleme durumu yakında burada gösterilecek.
              </div>
            </div>
            <Button>
      ...
      <Tabs defaultValue="progress" className="w-full">
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-2xl">
                    Ölçülebilir İlerleme ve İttifak Paneli
                  </CardTitle>
                  <CardDescription>
                    Danışanınızın standart ölçeklerdeki ilerlemesini ve
                    terapötik ittifak geri bildirimlerini takip edin.
                  </CardDescription>
                </div>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem
                      onClick={() => handleAssignAssessment('GAD-7')}
                    >
                      GAD-7 (Anksiyete)
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => handleAssignAssessment('PHQ-9')}
                    >
                      PHQ-9 (Depresyon)
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() =>
                        handleAssignAssessment('TherapeuticAlliance')
                      }
                    >
                      Terapötik İttifak
                    </DropdownMenuItem>
              ...
              {loadingAssessments ? (
                ...
                <div className="text-center text-muted-foreground py-16">
                  ...
                  <p className="text-sm mt-1">
                    Yukarıdaki butonu kullanarak bir değerlendirme
                    atayabilirsiniz.
                  </p>
                </div>
              )}
        ...
        <TabsContent value="briefing" className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-2xl">
                {clientData.displayName} ile Yaklaşan Seans Brifingi
              </CardTitle>
              <CardDescription>
                Tarih: {format(new Date(), 'd MMMM yyyy, EEEE', { locale: tr })}
                . Bu özet, seansa hazırlanmanıza yardımcı olmak için
                oluşturulmuştur.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-4">
                ...
                <Alert className="border-primary/50 bg-primary/10">
                  <AlertTitle className="font-bold">
                    Danışanınız bu konuyu seansta konuşmak istiyor:
                  </AlertTitle>
                  <AlertDescription>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-sm">
                      Son seanstan bu yana kriz sinyali{' '}
                      <span className="font-bold">tespit edilmedi</span>.
                    </p>
                  </CardContent>
                </Card>
              </div>
        ...
        <TabsContent value="journals" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Danışanın sizinle paylaşmayı seçtiği günlük kayıtları ve
                düşünceleri.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="results" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Danışanın tamamladığı testlerin sonuçlarını ve yapay zeka
                analizlerini görüntüleyin.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loadingResults ? (
                ...
                >
                  {testResults.map(result => (
                    <AccordionItem
                      value={result.id}
                      key={result.id}
                      className="border rounded-md px-4 bg-muted/20"
                    >
                      <AccordionTrigger>
                      ...
                      <AccordionContent className="space-y-4 pt-4">
                        <Alert>
                          ...
                          <AlertTitle>
                            {result.testName === 'Tükenmişlik Envanteri'
                              ? 'Risk Seviyesi'
                              : 'Şiddet'}
                          </AlertTitle>
                          <AlertDescription>
