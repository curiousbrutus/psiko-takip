...
export async function addAppointmentAction(input: AddAppointmentInput): Promise<{ success: boolean; message: string; }> {
  ...
  try {
    ...
    return { success: false, message: 'Randevu oluşturulurken bir hata oluştu.' };
  }
}


export async function getClientsForTherapistAction(therapistId: string): Promise<{ id: string; displayName: string; }[]> {
