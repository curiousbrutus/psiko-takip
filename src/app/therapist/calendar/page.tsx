...
const appointmentFormSchema = z.object({
  ...
  type: z.enum(['Online', 'Yüz Yüze'], { required_error: "Lütfen bir seans türü seçin." }),
...
export default function CalendarPage() {
    ...
    const fetchAppointments = async () => {
        ...
        try {
            ...
            toast({ title: "Hata", description: "Randevular alınamadı.", variant: "destructive" });
        } finally {
            setLoading(false);
        }
    };
    ...
    async function onSubmit(values: AppointmentFormValues) {
        ...
        if (!selectedClient) {
            toast({ title: "Hata", description: "Geçerli bir danışan seçilemedi.", variant: "destructive" });
            return;
        }
    ...
    const timeSlots = Array.from({ length: 20 }, (_, i) => { // 9:00 to 18:30
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                   Danışanınız için yeni bir seans planlayın. Seçilen tarih: {date ? format(date, "d MMMM yyyy", { locale: tr }) : ''}
                                </DialogDescription>
                            </DialogHeader>
                            <Form {...form}>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Textarea placeholder="Seans hakkında kısa notlar..." {...field} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1">
                    <Card>
                        <CardHeader>
                            <CardTitle>
                                {date ? format(date, "d MMMM yyyy, EEEE", { locale: tr }) : "Bir tarih seçin"}
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            {loading ? (
                                ...
                                <ul className="space-y-4">
                                    {selectedDayAppointments.map((app) => (
                                        <li key={app.id} className="flex items-start gap-4 p-3 bg-muted/50 rounded-lg border">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="text-sm text-muted-foreground flex items-center gap-1.5 mt-1">
                                                    {app.type === 'Online' ? (
                                                         <Badge variant="outline" className="text-primary border-primary/50"><Video className="mr-1 h-3 w-3"/> Online</Badge>
                                                    ) : (
                                                        <Badge variant="secondary"><Clock className="mr-1 h-3 w-3" />Yüz Yüze</Badge>
                                                    )}
                                                </div>
                                            </div>
                                        </li>
