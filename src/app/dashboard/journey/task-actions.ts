...
import { collection, query, where, getDocs, orderBy, DocumentData, collectionGroup } from 'firebase/firestore';
...
export async function getAssignedTasks(): Promise<{ success: boolean; data?: DocumentData[]; error?: string }> {
  ...
  try {
    ...
    const tasks = tasksSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), type: 'collaborative' }));
    const assessments = assessmentsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), type: 'assessment' }));
