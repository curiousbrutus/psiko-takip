...
const GratitudeSchema = z.object({
  content: z.string().min(3, "Anı en az 3 karakter olmalıdır.").max(500, "Anı en fazla 500 karakter olabilir."),
});
...
export async function getGratitudeEntries(): Promise<{ success: boolean; data?: DocumentData[]; error?: string }> {
  const user = auth.currentUser;
  if (!user) {
    return { success: false, error: "Giriş yapmalısınız." };
  }
