...
export default function DailyJourneyPage() {
  ...
  const updateGamificationStats = async (xp: number) => {
    ...
    try {
      ...
      toast({
        title: 'Hata',
        description: 'İlerlemeniz kaydedilemedi.',
        variant: 'destructive',
      });
    }
  };
  ...
  const completeTask = async (task: 'morning' | 'evening') => {
    if (!user) {
      toast({
        title: 'Giriş Gerekli',
        description:
          'Bu özelliği kullanmak için lütfen kayıt olun veya giriş yapın.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (task === 'morning') {
        if (!morningMood) {
          toast({
            title: 'Hata',
            description: 'Lütfen sabah ruh halinizi seçin.',
            variant: 'destructive',
          });
      ...
      if (task === 'evening') {
        if (!eveningMood) {
          toast({
            title: 'Hata',
            description: 'Lütfen akşam ruh halinizi seçin.',
            variant: 'destructive',
          });
      ...
      toast({
        title: 'Hata',
        description: 'Göreviniz kaydedilemedi. Lütfen tekrar deneyin.',
        variant: 'destructive',
      });
    } finally {
    ...
    <div className="space-y-6">
      ...
      {userData?.connectedTherapist && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Terapistinin senin için atadığı interaktif görevleri ve
              değerlendirmeleri buradan takip edebilirsin.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading.tasks ? (
      ...
      >
        <AccordionItem value="item-1" className="border-none">
          <Card>
            <AccordionTrigger
              ...
              disabled={tasksCompleted.morning}
            >
            ...
            <AccordionContent className="p-6 pt-0">
              <div className="space-y-6">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {moodOptions.map(mood => (
                      <Button
                        key={mood.name}
                        variant={
                          morningMood === mood.name ? 'default' : 'outline'
                        }
                ...
                <div>
                  ...
                  <Textarea
                    placeholder="Bugün kendim için yapacağım küçük bir şey... (Örn: 10 dakika mola vereceğim)"
                    value={morningNiyet}
                    onChange={e => setMorningNiyet(e.target.value)}
                  />
                </div>
                <Button
                  onClick={() => completeTask('morning')}
                  disabled={loading.morning || tasksCompleted.morning}
                >
                  {loading.morning && <Loader2 className="animate-spin mr-2" />}
                  {tasksCompleted.morning
                    ? 'Tamamlandı'
                    : 'Sabah Görevini Tamamla'}
                </Button>
              </div>
            </AccordionContent>
          </Card>
        ...
        <AccordionItem value="item-2" className="border-none">
          <Card>
            <AccordionTrigger
              className="p-6 hover:no-underline [&[data-state=open]>div>svg.lucide-check-circle-2]:hidden"
              disabled={tasksCompleted.evening}
            >
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-xl text-left">
                      Akşam Değerlendirmesi (+15 Enerji)
                    </CardTitle>
                    <p className="text-sm text-muted-foreground font-normal">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="font-semibold mb-2 flex items-center gap-2">
                    <Flame className="h-4 w-4 text-accent" /> Bugün minnettar
                    olduğun 3 şey nedir?
                  </h3>
                  <Textarea
                    placeholder="1. ..."
                    value={minnettar}
                    onChange={e => setMinnettar(e.target.value)}
                  />
                  {userData?.connectedTherapist && (
                    <div className="flex items-center space-x-2 mt-2">
                      <Switch
                        id="share-minnettar"
                        checked={isMinnettarShared}
                        onCheckedChange={setIsMinnettarShared}
                      />
                      <Label
                        htmlFor="share-minnettar"
                        className="text-sm text-muted-foreground flex items-center gap-1"
                      >
                        <Share2 className="h-3 w-3" /> Terapistle paylaş
                      </Label>
                    </div>
                  )}
                </div>
                <div>
                  <h3 className="font-semibold mb-2 flex items-center gap-2">
                    <Book className="h-4 w-4 text-accent" /> Serbest Günlük
                  </h3>
                  <Textarea
                    placeholder="Aklından geçenleri buraya yazabilirsin..."
                    value={gunluk}
                  ...
                  {userData?.connectedTherapist && (
                    <div className="flex items-center space-x-2 mt-2">
                      <Switch
                        id="share-gunluk"
                        checked={isGunlukShared}
                        onCheckedChange={setIsGunlukShared}
                      />
                      <Label
                        htmlFor="share-gunluk"
                        className="text-sm text-muted-foreground flex items-center gap-1"
                      >
                        <Share2 className="h-3 w-3" /> Terapistle paylaş
                      </Label>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {moodOptions.map(mood => (
                      <Button
                        key={mood.name}
                        variant={
                          eveningMood === mood.name ? 'default' : 'outline'
                        }
                        className="flex-col h-20 w-20"
                ...
                <Button
                  onClick={() => completeTask('evening')}
                  disabled={loading.evening || tasksCompleted.evening}
                >
                  {loading.evening && <Loader2 className="animate-spin mr-2" />}
                  {tasksCompleted.evening
                    ? 'Tamamlandı'
                    : 'Akşam Görevini Tamamla'}
                </Button>
              </div>
            </AccordionContent>
          </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Zihnini dinlendirecek ve ana odaklanmanı sağlayacak interaktif
            egzersizler.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Link
            href="/dashboard/journey/breathing-exercise"
            className="block p-4 border rounded-lg hover:bg-muted/50 transition-colors h-full"
          >
            ...
            <p className="text-sm text-muted-foreground mt-1 pl-8">
              Stresi azaltmak ve odaklanmak için yönlendirmeli nefes
              egzersizleri.
            </p>
          </Link>
          <Link
            href="/dashboard/journey/thought-bubbles"
            className="block p-4 border rounded-lg hover:bg-muted/50 transition-colors h-full"
          >
            <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground mt-1 pl-8">
              Düşüncelerini gözlemle ve sakince gitmelerine izin ver.
            </p>
          </Link>
          <Link
            href="/dashboard/journey/grounding-exercise"
            className="block p-4 border rounded-lg hover:bg-muted/50 transition-colors h-full"
          >
            <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground mt-1 pl-8">
              5 duyunla şimdiki ana demir at.
            </p>
          </Link>
          <Link
            href="/dashboard/journey/gratitude-jar"
            className="block p-4 border rounded-lg hover:bg-muted/50 transition-colors h-full"
          >
            <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground mt-1 pl-8">
              İyi anları biriktir ve dilediğinde hatırla.
            </p>
          </Link>
          <Link
            href="/dashboard/journey/zen-garden"
            className="block p-4 border rounded-lg hover:bg-muted/50 transition-colors h-full"
          >
            <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground mt-1 pl-8">
              Sanal kumda desenler çizerek zihnini sakinleştir.
            </p>
          </Link>
        </CardContent>
      </Card>
