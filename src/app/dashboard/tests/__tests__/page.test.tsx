...
// Mock the test-history API so the page's loading state resolves and the
// content (not the loading skeleton) renders during the test.
jest.mock('@/lib/api-client', () => ({
  apiGetTestSubmissions: jest
    .fn()
...
describe('TestsPage', () => {
  it('renders the page title correctly', async () => {
    ...
    // findByText waits for the async loading state to resolve
    expect(await screen.findByText('Mevcut Testler')).toBeInTheDocument();
  });
  ...
  it('renders the page description', async () => {
    ...
    expect(
      await screen.findByText(/Aşağıdaki testleri tamamladıktan sonra/)
    ).toBeInTheDocument();
  });

  it('renders all available tests', async () => {
    render(<TestsPage />);

    // Check for Beck Depression Inventory (now shows short title)
    expect(await screen.findByText('Beck Depresyon')).toBeInTheDocument();
  ...
  it('shows enabled and disabled test buttons', async () => {
    render(<TestsPage />);

    // Check that we have test buttons - both enabled and disabled
    const allTextNodes = await screen.findAllByText(/Teste Başla|Yakında/);
    expect(allTextNodes.length).toBeGreaterThanOrEqual(4); // Should have at least 4 buttons
  });

  it('has correct links for enabled tests', async () => {
    render(<TestsPage />);
  ...
  it('renders test descriptions correctly', async () => {
    render(<TestsPage />);

    expect(
      await screen.findByText(/Depresyonun şiddetini ölçmek için/)
    ).toBeInTheDocument();
  ...
  it('renders icons for each test', async () => {
    render(<TestsPage />);

    // Check that icons are rendered (mocked as strings)
    expect(
      (await screen.findAllByText('BrainCircuit')).length
    ).toBeGreaterThanOrEqual(3); // Beck, Young Schema, MMPI
    expect(screen.getAllByText('Flame').length).toBeGreaterThanOrEqual(1); // Burnout
  });

  it('renders correct test categories and structure', async () => {
    render(<TestsPage />);

    // Check the main content container exists
    const container = (
      await screen.findByText('Psikolojik Değerlendirmeler')
    ).closest('div');
